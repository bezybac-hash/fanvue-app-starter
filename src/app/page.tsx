...
export default async function Home({ searchParams }: { searchParams?: { [key: string]: string | string[] | undefined } }) {
  ...
  return (
    <div className="font-sans grid grid-rows-[20px_1fr_20px] items-center justify-items-center min-h-screen p-8 pb-20 gap-16 sm:p-20">
      <main className="flex flex-col gap-[24px] row-start-2 items-center sm:items-start max-w-[1200px] w-full">
        ...
        <div className="w-full">
          <div className="mb-4">
            <h2 className="text-2xl font-bold mb-2">{t('creatorsTitle')}</h2>
            <p className="text-sm text-black/70 dark:text-white/70">{t('creatorsDescription')}</p>
          </div>
          <CreatorsList creators={mockCreators} />
        </div>
      </main>
